...
$FileURL="https://s3.wasabisys.com/XXXX/Windows1909.iso"
$FileOutput="C:\output\Windows1909.iso"
...
if (Test-Path $FileOutput) {
  ...
  if (Test-Path $SetupPath) {
    ...
    $SetupProcess = Start-Process `
           ...
           -RedirectStandardError c:\output\win_error.log `
           -RedirectStandardOutput c:\output\win_output.log `
           -Passthru `
           -Wait
    ...
    # IF NO ERROR CODE
      ...
      $Settings = New-ScheduledTaskSettingsSet -DontStopIfGoingOnBatteries -DeleteExpiredTaskAfter (New-Timespan -Days 2)
      $User = "NT AUTHORITY\SYSTEM"
      $Argument = "/auto Upgrade /quiet /DynamicUpdate enable"
      ...
      if (Register-ScheduledTask -TaskName "Upgrade Windows 10" -Trigger $Trigger -User $User -Action $Action -RunLevel Highest -Force) {
